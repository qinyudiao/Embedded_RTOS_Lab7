...
int ADC_Collect4Chan(uint32_t channels[], uint32_t fs, void (*handler)(unsigned long))
{
  ...
  // use MUX0 - MUX3, where MUX0 is the first channel
  ADC0_SSMUX0_R = channels[0] | (channels[1] << 4) | (channels[2] << 8) | (channels[3] << 12);

  // currently assuming MUX0 corresponds to the first 4 bits in CTL reg (also corresponding order in FIFO)
  // cannot find exact info in the datashhet
  ADC0_SSCTL0_R = 0x04 | (0x04 << 4) | (0x04 << 8) | (0x06 << 12);
